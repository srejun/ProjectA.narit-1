exports.go= async function (req, res) {
    ...
    const col_user="user"
    const DB="weather"
    var Bres ={confirm: false,err:null}
  ...
  MongoClient.connect(url,async function (err, db) {
    if (err) throw err;
    var dbo = db.db(DB);
    have_ses = await dbo.collection(col_ses).find({session_id:req.sessionID,name:myobj.name}).toArray() ////check session
    ...
    if(have_ses<=0&&have_use.length<=0)
        {dbo.collection(col_user).insertOne(myobj),Bres.confirm=true         ///insert user                                     
         console.log('insert complete') }
    else if(have_ses<=0&&have_use.length>0){console.log('same user'),Bres.err='same user'}
    else {console.log('you are login now'),Bres.err='same session'}
