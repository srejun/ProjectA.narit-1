    ...
    if (req.body['location'] === undefined || req.body['inBuilding'] === undefined || req.body['Fyear'] === undefined || req.body['Tyear'] === undefined) {
        ...
        MongoClient.connect(url, async function (err, db) {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                data[new Date(result[i].date).getFullYear()][new Date(result[i].date).getMonth() + 1].ave.time = new Date(result[i].date).getFullYear()+ "/" + parseInt(new Date(result[i].date).getMonth()+1)
